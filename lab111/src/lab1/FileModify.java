...
//  public static String[][][] jingguo = new String[1000][1000][1000];
   ...
   public static int[][] w1 =new int[10000][10000];
...
//       int t = wordList.indexOf(word);
   ...
   {
       String[] cu = {"if","not","the","sun","for","smiling","to","if","turned","the","to","found","at","we","now"};
...
//           System.out.println(".");
   ...
   public static void generateNewText()//yipipei现在是全局变量
   {
       String yh_out = new String();//用来输出由用户输入的文本所补充出的新文本内容

       for(int i=0;i<yhglob-1;i++)
       {
           queryBridgeWords(yhcun[i],yhcun[i+1]);
           if(yipipei==1)//////////////////////////////////////////////////////////////////////////////////////////////////!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           {
               yh_out=String.format("%s "+"%s "+"%s ",yh_out ,yhcun[i] , first_Bridge);
           }
           else
           {
               yh_out=String.format("%s "+"%s ",yh_out , yhcun[i]);
           }

   //插入该桥连接词
       }
       yh_out=String.format(yh_out + yhcun[yhglob-1]);
